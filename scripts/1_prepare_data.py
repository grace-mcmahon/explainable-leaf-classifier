...
def main(): # def defines the function so everytime we call main what happens inside of this block will run. Main is given as the 'function that runs everything'
    ...
    if not RAW_DATA_DIR.exists():
        raise FileNotFoundError(
            f"Couldn't find {RAW_DATA_DIR}. Download the dataset from Kaggle,"
            f"unzip it, and place/point it at this folder first"
        )
    ...
    for class_dir in class_dirs:
        ...
        # We loop over each class .glob("*.*") finds every file inside it with any extension (the images). list(...) converts that into an actual list we cna work with. random.shuufle() mixes up the order - important so the split isnt accidently biased (e.g. all the blurry photos happening at the end.)
        n_total = len(images)
